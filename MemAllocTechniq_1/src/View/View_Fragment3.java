			...
			if (unit.status == true) 
			{
               ...
               // size+=unit.getRemaining_size();
                if(unit.getProcess_id()!=tmp){
    				System.out.println("Worst Fit:Allocated memory block no. "+unit.getFragment_id()+" with size "+unit.getMemsize()+" to process P"+unit.getProcess_id()+" with size "+(unit.getMemsize()-unit.getRemaining_size()));
